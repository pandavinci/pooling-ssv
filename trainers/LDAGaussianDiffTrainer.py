...
class LDAGaussianDiffTrainer(BaseSklearnTrainer):
    def __init__(self, model: LDAGaussianDiff, device="cuda" if torch.cuda.is_available() else "cpu", save_path=None):
        super().__init__(model, device, save_path)
        self.model: LDAGaussianDiff  # Type hinting so Pylance and Pyright don't complain
